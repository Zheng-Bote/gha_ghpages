...
public:
    ...
    std::string description() const override { return "Processes GFM-style alerts and specific emojis."; }
...
private:
    void process_alerts(std::string& html) {
        ...
        for (const auto& [type, css_class] : alerts) {
            ...
            while ((search_pos = html.find(marker, search_pos)) != std::string::npos) {
                ...
                if (search_pos + erase_len < html.length() && html[search_pos + erase_len] == ' ') {
                    erase_len++;
                }
    ...
    /**
     * @brief Finds the matching </blockquote> for an opening tag, handling nesting.
     */
