  ...
  std::map<std::string, std::string> parse_args(const std::string &args_str) {
    ...
    while (std::getline(ss, segment, ',')) {
      ...
      if (eq_pos != std::string::npos) {
        ...
        // FIX: Create proper positional argument keys
...
public:
  ...
  ShortcodePlugin() {
    handlers["youtube"] = [](const auto &args) -> std::string {
      std::string id = args.count("id")
                           ? args.at("id")
                           : (args.count("_0") ? args.at("_0") : "");
      if (id.empty())
        return "<!-- Missing YouTube ID -->";
      // FIX: Removed unnecessary escapes from raw string
      return std::format(
    ...
    handlers["alert"] = [](const auto &args) -> std::string {
      ...
      std::string text = args.count("text")
                             ? args.at("text")
                             : (args.count("_0") ? args.at("_0") : "");
      std::string class_name = "info-msg";
      if (type == "warning")
        class_name = "warning-msg";
      else if (type == "error")
        class_name = "error-msg";
      else if (type == "success")
        class_name = "success-msg";
    ...
    handlers["button"] = [](const auto &args) -> std::string {
      ...
      std::string text = args.count("text")
                             ? args.at("text")
                             : (args.count("_0") ? args.at("_0") : "Click");
      ...
      // FIX: Removed backticks and unnecessary escapes
      return std::format(
          R"(<a href="{}" class="btn btn-{}" alt="{}" title="{}">{}</a>)", url,
          type, alt, title, text);
    };
  ...
  void on_before_render(model::PageContext &ctx) override {
    ...
    while ((start_pos = content.find("[[", last_pos)) != std::string::npos) {
      ...
      // 2. Find matching ]] (FIX: was searching for ]]])
      size_t end_pos = content.find("]]", start_pos + 2);
      if (end_pos == std::string::npos) {
        // Malformed, no closing ]], preserve [[ and continue
        result.append("[[");
      ...
      std::string inner =
          content.substr(start_pos + 2, end_pos - (start_pos + 2));
      ...
      // 5. If it's not a recognized shortcode, preserve the entire block
      // FIX: Preserve full shortcode instead of just [[
      result.append(content, start_pos, end_pos + 2 - start_pos);
      last_pos = end_pos + 2;
    }
