...
public:
    ...
    std::string description() const override { return "Renders mermaid code blocks as diagrams"; }
    ...
    bool on_md_block_enter(MD_BLOCKTYPE type, void* detail, model::PageContext& ctx, std::ostream& out) override {
    ...
    bool on_md_text(MD_TEXTTYPE type, const char* text, MD_SIZE size, model::PageContext& ctx, std::ostream& out) override {
        if (in_mermaid_block) {
    ...
    bool on_md_block_leave(MD_BLOCKTYPE type, void* detail, model::PageContext& ctx, std::ostream& out) override {
    ...
    void on_after_render(model::PageContext& ctx) override {
        ...
        if (ctx.plugin_data.contains("has_mermaid") && std::any_cast<bool>(ctx.plugin_data["has_mermaid"])) {
            std::string script = R"(
<script type="module">
