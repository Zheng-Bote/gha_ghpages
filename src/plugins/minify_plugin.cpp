...
public:
    ...
    std::string description() const override { return "Minifies HTML and CSS output"; }

    void on_post_build(model::SiteContext& ctx) override {
        std::cout << "Minifying assets in " << ctx.output_root << "..." << std::endl;
        
        size_t count = 0;
        for (auto& p : std::filesystem::recursive_directory_iterator(ctx.output_root)) {
...
private:
    void minify_file(const std::filesystem::path& path, std::string (MinifyPlugin::*minifier)(const std::string&)) {
    ...
    std::string minify_html(const std::string& input) {
        ...
        // 1. Remove comments <!-- ... -->
        // Note: This removes all comments. If IE conditionals are needed, this regex needs adjustment.
        s = std::regex_replace(s, std::regex(R"(<!--[\s\S]*?-->)"), "");
        
        // 2. Collapse whitespace between tags: >  < becomes ><
        // This is generally safe for block elements, but can affect inline-block layout.
        // However, for a standard doc site, it usually works fine and saves space.
        s = std::regex_replace(s, std::regex(R"(>\s+<)"), "><");
