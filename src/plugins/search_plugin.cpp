...
public:
    ...
    std::string description() const override { return "Generates search.json for client-side search"; }
    ...
    void on_after_render(model::PageContext& ctx) override {
        ...
        // 3. Content
        // We use the HTML content because it has the final text (after markdown/asciidoc processing)
    ...
    void on_post_build(model::SiteContext& ctx) override {
        ...
        if (out) {
            ...
            std::cout << "Generated search.json with " << entries.size() << " entries." << std::endl;
        } else {
            std::cerr << "Failed to write search.json to " << out_file.string() << std::endl;
        }
    }
};
