...
class MarkdownRenderer : public core::IRenderer {
    ...
    static int enter_block(MD_BLOCKTYPE type, void* detail, void* userdata) {
        ...
        if (state->host.dispatch_md_enter(type, detail, state->ctx, state->out)) return 0;

        switch(type) {
            case MD_BLOCK_DOC: break; // Root
            case MD_BLOCK_QUOTE: state->out << "<blockquote>\n"; break;
            case MD_BLOCK_UL: state->out << "<ul>\n"; break;
            case MD_BLOCK_OL: state->out << "<ol>\n"; break;
            case MD_BLOCK_LI: state->out << "<li>"; break;
            case MD_BLOCK_HR: state->out << "<hr>\n"; break;
            case MD_BLOCK_H: {
                auto* h_detail = static_cast<MD_BLOCK_H_DETAIL*>(detail);
                state->out << "<h" << h_detail->level << ">"; 
                break;
            }
            case MD_BLOCK_CODE: state->out << "<pre><code>"; break;
            case MD_BLOCK_P: state->out << "<p>"; break;
            default: break;
        }
        return 0;
    }
    ...
    static int leave_block(MD_BLOCKTYPE type, void* detail, void* userdata) {
        auto* state = static_cast<RenderState*>(userdata);
        if (state->host.dispatch_md_leave(type, detail, state->ctx, state->out)) return 0;
        ...
        switch(type) {
            ...
            case MD_BLOCK_UL: state->out << "</ul>\n"; break;
            case MD_BLOCK_OL: state->out << "</ol>\n"; break;
            case MD_BLOCK_LI: state->out << "</li>\n"; break;
            case MD_BLOCK_H: {
                auto* h_detail = static_cast<MD_BLOCK_H_DETAIL*>(detail);
                state->out << "</h" << h_detail->level << ">\n";
                break;
            }
            case MD_BLOCK_CODE: state->out << "</code></pre>\n"; break;
            case MD_BLOCK_P: state->out << "</p>\n"; break;
            default: break;
        }
        return 0;
    }
    ...
    static int enter_span(MD_SPANTYPE type, void* detail, void* userdata) {
        auto* state = static_cast<RenderState*>(userdata);
        // For simplicity, spans are not currently dispatched to plugins (can be added)
        
        switch(type) {
            case MD_SPAN_EM: state->out << "<em>"; break;
            case MD_SPAN_STRONG: state->out << "<strong>"; break;
            case MD_SPAN_A: {
            ...
            case MD_SPAN_CODE: state->out << "<code>"; break;
            default: break;
        }
        return 0;
    }
    ...
    static int leave_span(MD_SPANTYPE type, void* detail, void* userdata) {
        ...
        switch(type) {
            case MD_SPAN_EM: state->out << "</em>"; break;
            case MD_SPAN_STRONG: state->out << "</strong>"; break;
            case MD_SPAN_A: state->out << "</a>"; break;
            case MD_SPAN_CODE: state->out << "</code>"; break;
            default: break;
        }
        return 0;
    ...
    static int text(MD_TEXTTYPE type, const char* text, MD_SIZE size, void* userdata) {
        auto* state = static_cast<RenderState*>(userdata);
        if (state->host.dispatch_md_text(type, text, size, state->ctx, state->out)) return 0;
        ...
        if (type == MD_TEXT_HTML || type == MD_TEXT_ENTITY) {
            ...
            for (MD_SIZE i = 0; i < size; ++i) {
                switch(text[i]) {
                    ...
                    case '&': state->out << "&amp;"; break;
                    case '"': state->out << "&quot;"; break;
                    default: state->out << text[i]; break;
                }
            }
        }
        return 0;
...
public:
    ...
    std::expected<std::string, std::string> render(model::PageContext& ctx) override {
        ...
        int ret = md_parse(ctx.raw_content.c_str(), ctx.raw_content.size(), &parser, &state);
        
        if (ret != 0) return std::unexpected("Markdown parsing failed");
        return out.str();
    }
};

std::unique_ptr<core::IRenderer> MarkdownPlugin::create_renderer(const std::string& extension) {
